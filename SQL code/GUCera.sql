--max grade in both assignment and course, add rate, pay course
...
go
declare @id int
exec studentRegister 'k','g','123','n',1,'auc',@id output
print(@id)
go 
...
as
begin
if @ID is not null and @mobile_number is not null and exists(select * from Users where id = @ID and password = @password)
...
As
Select id,name, creditHours, price, content, accepted
From Course
go
...
Insert into Course(creditHours,name,price,instructorId,accepted,content,courseDescription) values
(@creditHours,@name,@price,@instructorId,0,'','')
declare @cid int
...
if(exists(select * from StudentTakeCourse where sid=@sid and (grade <2.0 or grade is null) and cid=@cid))
set @check = 1
...
SELECT c.name, c.id 
FROM Course c
WHERE c.accepted = '1' AND not exists(
select *
from StudentTakeCourse
where sid = @sid and cid = c.id
)
go
...
SELECT x.* , c.grade
FROM StudentCertifyCourse x
inner join StudentTakeCourse c on c.cid = x.cid and c.sid = x.sid
WHERE x.sid = @sid AND x.cid = @cid
ELSE
...
go
create proc getCourseName
@courseID int,
@courseName varchar(10) output
as
...
go
create proc ShowInstructorsNotTeachingCourse
@cid int
as
select I.id as id, u.firstName + ' ' + u.lastName as name
from Instructor I
inner join Users u on u.id = I.id
where not exists(
select *
from InstructorTeachCourse
where cid = @cid and I.id= insid
)

go
create proc ShowNonPrerequisitedCourses
@cid int
as
select id, name
from Course
where id <> @cid and accepted =1 and not exists(
select *
from CoursePrerequisiteCourse
where cid = @cid and id = preid 
)
go
create proc ShowCoursesAndStudents
@instID int
as
select s.id as studentID, u.firstName+' '+u.lastName as studentName, c.id as courseID, c.name as courseName
from StudentTakeCourse l
inner join Student s on s.id = l.sid
inner join Course c on c.id = l.cid
inner join Users u on u.id = s.id
where l.insid = @instID and not exists(
select *
from StudentCertifyCourse x
where x.sid = s.id and x.cid = c.id
)

go
CREATE FUNCTION InstTeachCourse
(@inst INT, @cid INT)
RETURNS BIT
BEGIN
DECLARE @returnValue BIT
IF(EXISTS(SELECT * FROM InstructorTeachCourse WHERE insid=@inst  AND cid = @cid))
SET @returnValue = '1'
ELSE
SET @returnValue = '0'
RETURN @returnValue
END
go
create proc showInstructorTeachCourse
@cid int
as
select i.id, u.firstName+' '+u.lastName as name
from Instructor i
inner join Users u on i.id = u.id
inner join InstructorTeachCourse x on i.id = x.insid
where x.cid = @cid
go
...
inner join Course c on c.id = stc.cid
where stc.sid = @sid
go
create proc ViewNotYetSubmittedAssignments
@sid int
as
select type, number, c.id,c.name
from Assignment a
inner join Course c on c.id = a.cid
where exists(select * from StudentTakeCourse tmp where tmp.cid = c.id and tmp.sid = @sid)
and not exists(select * from StudentTakeAssignment tmp 
where tmp.assignmentNumber = a.number 
and tmp.assignmenttype = a.type
and a.cid = tmp.cid
and tmp.sid = @sid
)
go
create proc ViewSubmittedAssignments
@sid int
as
select assignmenttype as type, assignmentNumber as number, c.id,c.name
from StudentTakeAssignment x
inner join Course c on c.id = x.cid
where x.grade is not null and x.sid = @sid
go
create proc ViewCoursesCertified
@sid int
as
select c.id, c.name
from StudentCertifyCourse x
...
where x.sid = @sid
go
create proc ShowAllStudent
as
Select u.id, u.firstName+' '+u.lastName as name
from Student s
inner join Users u on u.id = s.id
go
create proc ShowNonAssignedPromoCodes
as
select p.code
from Promocode p
where not exists(
select *
from StudentHasPromocode x
where p.code = x.code
)